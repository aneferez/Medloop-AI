...
// { patient, device, user } or throws 401. `user` is the server-side account
// (present for /auth/signup accounts); it is null for legacy device-token-only
// accounts, which still authenticate. Tokens are stored only as SHA-256 hashes;
...
export async function authenticate(request, db) {
  ...
  let user = null
  const userId = device.user_id || patient.owner_user_id
  if (userId) user = await db.first('SELECT * FROM users WHERE id = ?', [userId])
