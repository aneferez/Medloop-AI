...
export function usePushRegistration({ user, accountReady, onAlert, attestationToken = '' }) {
  ...
  useEffect(() => {
    ...
    let active = true
    let cleanup = () => {}
    initPushNotifications(user, { onStatus: (next) => { if (active) setStatus(next) }, onAlert, attestationToken })
  ...
  }, [user?.uid, user?.email, accountReady, onAlert, attestationToken])
